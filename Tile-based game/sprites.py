...
class Player(pg.sprite.Sprite):
    def __init__(self, game, x, y):
        ...
        pygame.draw.circle(self.image, (255, 255, 25, 0), self.rect.center, 20)
    ...
    def shoot(self):
        ...
        if now - self.last_shot > WEAPONS[self.weapon]['rate']:
            ...
            spawn_pos = self.pos + BARREL_OFFSET.rotate(-self.rot)
            self.vel = vec(-WEAPONS[self.weapon]['kickback'], 0).rotate(-self.rot)
            for i in range(WEAPONS[self.weapon]['count']):
                spread = uniform(-WEAPONS[self.weapon]['spread'], WEAPONS[self.weapon]['spread'])
                Bullet(self.game, spawn_pos, dir.rotate(spread))
            MuzzleFlash(self.game, spawn_pos)
    ...
    def update(self):
        self.get_keys()
        self.mouse_pos_vec = vec(pg.mouse.get_pos()) - vec(self.game.camera.apply(self).center)
        self.rot = self.mouse_pos_vec.angle_to(vec(1, 0))
...
class Mob(pg.sprite.Sprite):
    ...
    def update(self):
        # set initial acc to 0
        self.acc = vec(0, 0)
        # if player detected, apply normal acc
        ...
        if self.health <= 0:
            ...
            self.kill()
            self.game.map_img.blit(choice(self.game.zombie_splats), self.pos - vec(32, 32))
    ...
    def draw_health(self):
        ...
        if 0 < self.health < ZOMBIE_HEALTH:
...
class Bullet(pg.sprite.Sprite):
    def __init__(self, game, pos, dir):
        ...
        self.dmg = WEAPONS[game.player.weapon]['damage']
        self.knockback = WEAPONS[game.player.weapon]['knockback']
        self.dir = dir
        self.vel = self.dir * WEAPONS[game.player.weapon]['speed'] * uniform(0.9, 1.1)
